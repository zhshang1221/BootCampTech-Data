...
const UIColor = () => {
    ...
    const loadContract = async () => {
        ...
        const contractInfo = new web3.eth.Contract(abiInfo, '0x2e8A922D060746D0Bac9589D24a1029d42378D7B');
    ...
    const showNumber = async () => {
        const newNumber = await contract.methods.retrieve().call();
        console.log('This is number in contract', newNumber);
        setNumber(newNumber);
    }
    ...
    const submitStorage = () => {
        try {
            contract.methods.store(storage).send({ from: account }).on("transactionHash", (hash) => {
                ...
                <Grid item xs={12}>
                    <SubCard title="Primary Color">
                        <Grid container spacing={gridSpacing}>
                            ...
                            <Grid item xs={12} sm={6} md={4} lg={2}>
                                <Card>This is number in storage: {number}</Card>
                            </Grid>
                            <Grid item xs={12} sm={6} md={4} lg={2}>
                                <Button onClick={submitStorage}>Store number</Button>
                            </Grid>
                            <Grid item xs={12} sm={6} md={4} lg={2}>
                                <TextField id="outlined-basic" label="Outlined" variant="outlined" onChange={changeStorage}
                                />
                            </Grid>
                            <Grid item xs={12} sm={6} md={4} lg={2}>
